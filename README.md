# Building-an-Inclusive-Culture-Together
This digital companion is designed to help you transition from a passive observer to an active architect of an inclusive culture.
   <!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        /* Ensures charts are responsive and constrained */
        .chart-container {
            position: relative;
            width: 100%;
            max-width: 100%; 
...
<body class="flex h-screen overflow-hidden">
    ...
    <script>
        ...
        // --- GEMINI API CALL UTILITY (with error handling for static hosts) ---
        async function callGeminiApi(payload) {
            // Note: On static hosts like GitHub Pages, the placeholder apiKey="" will fail.
            // The user must either use a backend proxy or manually paste a key if allowed.
            const apiKey = ""; 
            const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`;
            const maxRetries = 5;

            // Check if API key is likely missing/placeholder and throw a user-friendly error
            if (apiKey === "") {
                 throw new Error("API_KEY_MISSING: LLM features require a valid API key. This host does not support automatic injection. You must manually add a key or use a proxy.");
            }
        ...
        async function generateInclusiveFeedback() {
            ...
            button.innerHTML = '<span class="animate-spin mr-2 inline-block">&#9696;</span> Drafting...';
            output.innerHTML = '';
            ...
            try {
                ...
                if (message.includes("API_KEY_MISSING")) {
                    message = "LLM feature disabled: API Key is required for this host. See console for details.";
                } else if (message.includes("400")) {
                     message = "API Key error: Check your API key or ensure the service is enabled.";
                }
                output.innerHTML = `<p class="text-red-600">${message}</p>`;
        ...
        async function defineAccountabilityMetrics() {
            ...
            button.disabled = true;
            button.innerHTML = '<span class="animate-spin mr-2 inline-block">&#9696;</span> Analyzing Goal...';
            output.innerHTML = '';
            statusDiv.classList.add('hidden');

            try {
                const systemPrompt = "You are a specialist in DEI Accountability and Metrics. The user will provide an Action Learning Project (ALP) goal. Your task is to generate 3 to 5 highly specific, measurable, achievable, relevant, and time-bound (SMART) Key Performance Indicators (KPIs) for this project, focusing on systemic inclusion change. Format the output as a clean, bulleted list of KPIs (using Markdown stars).";
        ...
        function renderSession4() {
            ...
            main.innerHTML = `
                <div class="fade-in max-w-6xl mx-auto">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- LLM Feature: ALP Metrics Designer -->
            ...
            // Initialize Charts on view load
            initRadarChart(data);
            initPlotlyChart(data);
        }

        // --- CHART INITIALIZATION FUNCTIONS ---

        function initMaturityChart(data) {
            // Use setTimeout to ensure Chart.js is ready, although window.onload should handle this
            setTimeout(() => {
        ...
        // This ensures all external libraries (Chart.js, Plotly, Tailwind) and the DOM are fully loaded
        // before we try to render the first view and initialize the charts.
